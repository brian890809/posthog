...
class TestSavedQuery(APIBaseTest):
    ...
    def test_update_sync_frequency_on_tiered_v2_writes_target_through(self):
        ...
        self.assertEqual(updated.sync_frequency_interval, timedelta(hours=24))
        node = Node.objects.get(saved_query_id=saved_query["id"])
        self.assertEqual(get_frequency_target(node), timedelta(hours=24))
