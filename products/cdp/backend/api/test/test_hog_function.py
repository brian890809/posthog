...
class TestLogTransformationAPI(ClickhouseTestMixin, APIBaseTest, QueryMatchingTest):
    ...
    @parameterized.expand([("fetch", "fetch('https://example.com')"), ("postHogCapture", "postHogCapture({})")])
    def test_rejects_async_functions_in_hog_code(self, _name, call):
        response = self._create_log_transformation(hog=f"{call}\nreturn record")
        assert response.status_code == status.HTTP_400_BAD_REQUEST, response.json()

    @parameterized.expand(
        [
            ("event", "return event"),
            ("person_field", "let e := person.properties.email\nreturn record"),
        ]
    )
    def test_rejects_unavailable_globals_in_hog_code(self, _name, hog):
        response = self._create_log_transformation(hog=hog)
        assert response.status_code == status.HTTP_400_BAD_REQUEST, response.json()
        assert "Variable not available in log transformations" in response.json()["detail"]
