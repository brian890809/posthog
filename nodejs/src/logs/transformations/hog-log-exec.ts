...
function encodeAttributeMap(map: Record<string, string>): Record<string, string> {
    const out: Record<string, string> = {}
    for (const [key, value] of Object.entries(map)) {
...
export function applyTransformResult(record: LogRecord, execResult: unknown): 'mutated' | 'dropped' | 'invalid' {
    ...
    if (attributes !== undefined) {
        record.attributes = attributes === null ? null : encodeAttributeMap(attributes)
    }
    if (resourceAttributes !== undefined) {
        record.resource_attributes = resourceAttributes === null ? null : encodeAttributeMap(resourceAttributes)
    }
